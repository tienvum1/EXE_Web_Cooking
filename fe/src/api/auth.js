...
const API_URL = process.env.REACT_APP_BACKEND_API_URL || 'https://localhost:4567';
...
export const setPassword = async (passwordData, token = null) => {
    ...
    const response = await axios.post(`${API_URL}/api/auth/set-password`, passwordData, config);
    return response.data;
};
...
export const resetPassword = async (token, newPassword) => {
  try {
    const response = await axios.post(`${API_URL}/api/auth/reset-password/${token}`, { password: newPassword });
