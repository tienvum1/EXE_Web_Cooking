...
export const fetchRecipePendingById = async (id) => {
  const res = await axios.get(`${API_URL}/api/recipes/pendings/${id}`, { withCredentials: true });
  return res.data;
};
...
export const fetchNewestRecipes = async () => {
  const res = await axios.get(`${API_URL}/api/recipes/newest`, { withCredentials: true });
  return res.data;
};

export const fetchMostLikedRecipes = async () => {
  const res = await axios.get(`${API_URL}/api/recipes/most-liked`, { withCredentials: true });
  return res.data;
};
...
export const fetchPendingRecipes = async () => {
  ...
  const res = await axios.get(`${API_URL}/api/recipes/pendings`, { withCredentials: true });
  return res.data;
};
...
export const updateRecipeStatus = async (id, status) => {
  // Assuming auth token is handled by axios interceptor or similar, or sent via cookie
  const res = await axios.put(`${API_URL}/api/recipes/${id}/status`, { status }, { withCredentials: true });
  return res.data;
};
...
export const getAIMenuSuggestion = async (prompt) => {
  try {
    ...
    const response = await axios.post(`${API_URL}/api/ai/menu-suggestion`, { prompt }, {
